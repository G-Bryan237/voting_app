...
class Candidate(AbstractBaseModel):
    ...
    def __init __(self, candidate_id = None, candidate_name = None, candidate_post = None, id_session = None ):
    ...
    def save(self): 
         if self.candidate_id: 
            ...
            query = f"INSERT INTO {self.__class__.TABLE_NAME} (candidate_name,candidate_post, id_session candidate_class,) VALUES(?,?,?,?)"
            with sqlite3.connect("db.sqlite") as connection:
                cursor = connection.cursor()
    ...
    def read(candidate_id=None):
        with sqlite3.connect("db.sqlite") as connection:
            cursor = connection.cursor()++++++++++++++++++++++++++++++++++++++++
            if candidate_id:
                query = f"SELECT (candidate_id, candidate_name, post, id_session) FROM {self.__class__.TABLE_NAME} WHERE candidate_id=?"
